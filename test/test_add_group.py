...
from model.group import Group
    ...
    # it could be error if data_gorup , but not data_groups
        ...
        # less 7.3 def test_add_group(app, json_groups):
def test_add_group(app, db, json_groups):
    group = json_groups
            # -revoke less 6.3for group in testdata:
            # less 5.4 def test_add_group(app):
            # less 7.3 old_groups = app.group.get_group_list()
    ...
    # less 7.3 assert len(old_groups) + 1 == app.group.count()
            # less 7.3 new_groups = app.group.get_group_list()
    new_groups = db.get_group_list()
    old_groups.append(group)
    assert sorted(old_groups,key=Group.id_or_max) == sorted(new_groups,key =Group.id_or_max)
      # if assert failed, then due to ' sign
